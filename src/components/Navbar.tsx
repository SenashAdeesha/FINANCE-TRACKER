...
function Navbar({ onMenuClick, pageTitle, titleColor, navbarGradient, underlineColor, onNotificationClick }: Props) {
  ...
  const [user, setUser] = useState<{ first_name?: string; last_name?: string; profile_picture?: string } | null>(null);
  ...
  return (
    <header className={`h-20 shadow-xl border-b flex items-center justify-between px-6 relative z-20 backdrop-blur-3xl ${navbarGradient || 'bg-white/70 border-gray-200'
      }`}>
      {/* Decorative overlay */}
      {!hasGradient && (
      ...
      <div className="flex items-center gap-4 relative z-10">
        ...
        {pageTitle && (
          <div>
            <h1 className={`text-3xl md:text-4xl font-extrabold ${titleColor || (hasGradient ? 'text-white drop-shadow-lg' : 'bg-gradient-to-r from-blue-600 via-purple-600 to-pink-600 bg-clip-text text-transparent')
              }`}>
              {pageTitle}
            </h1>
            {!hasGradient && (
        ...
        >
          {user?.profile_picture ? (
            <img 
              ...
              alt="Profile" 
              className="w-11 h-11 rounded-xl object-cover shadow-xl border-2 border-white/50"
            />
          ) : (
            <div className={`w-11 h-11 rounded-xl flex items-center justify-center text-sm font-bold shadow-xl ${hasGradient ? 'bg-white/20 text-white backdrop-blur-sm' : 'bg-gradient-to-br from-blue-500 via-purple-600 to-pink-600 text-white'
              }`}>
              {getInitials()}
            </div>
          )}
          <span className={`hidden md:block font-bold ${textColor}`}>{getDisplayName()}</span>
        </button>
